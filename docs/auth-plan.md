...
- [ ] Создать или проверить проект в Supabase.
- [ ] Включить нужные auth providers:
  - [ ] email/password;
  - [ ] Google OAuth.
- [ ] Настроить redirect URLs для локальной разработки и продакшена.
- [ ] Создать админский аккаунт в Supabase Auth.
- [ ] Зафиксировать один разрешенный Google-аккаунт для админа:
  - [ ] выбрать allowlist по `email`;
  - [ ] или allowlist по `user.id`;
  - [ ] или `app_metadata.role = admin`.
- [ ] Определить, где хранится allowlist и кто его проверяет: приложение, SQL-функция или оба слоя.

## 2. Обновить схему БД

- [ ] Добавить в основную таблицу поле `isDemo boolean not null default false`.
- [ ] Проверить, что все демо-записи помечены `isDemo = true`.
- [ ] Проверить, что все приватные записи помечены `isDemo = false`.
...
- [ ] Перед рискованными шагами сделать backup / export в Supabase.
